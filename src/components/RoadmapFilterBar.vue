...
<script>
const STORAGE_KEY = 'roadmap-filter'

export default {
  name: 'RoadmapFilterBar',
  props: {
    issues: {
      type: Array,
      required: true,
  ...
  methods: {
    ...
    loadFilter() {
      try {
        ...
        console.error('Failed to load roadmap filter from localStorage:', error)
      }
    },
    saveFilter() {
      try {
        ...
        console.error('Failed to save roadmap filter to localStorage:', error)
      }
    },
    emitFilterChange() {
      this.saveFilter()
