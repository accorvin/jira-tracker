  ...
  <main class="container mx-auto px-6 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md">
      ...
      <div v-if="activeTab === 'pending'" class="p-6">
        ...
        <div v-else-if="allProposals.length === 0" class="text-center py-8 text-gray-500">
          <p>No proposals.</p>
        ...
        <div v-else>
          ...
          <div class="flex items-center gap-3 mb-4">
            ...
            <select
              ...
              class="ml-auto px-2 py-1 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-primary-500"
            >
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-gray-200">
                ...
                <th class="text-left py-2 font-medium text-gray-600">Outcome</th>
              </tr>
            </thead>
            <tbody>
              <template
...
export default {
  ...
  computed: {
    filteredProposals() {
      if (this.statusFilter === 'all') return this.allProposals
      return this.allProposals.filter(p => p.status === this.statusFilter)
    },
    selectableProposals() {
      return this.filteredProposals.filter(p => this.isSelectable(p))
    },
    pendingCount() {
    ...
    allSelected() {
      return this.selectableProposals.length > 0 &&
        this.selectedIds.length === this.selectableProposals.length
    },
    someSelected() {
  ...
  methods: {
    ...
    toggleSelectAll() {
      if (this.allSelected) {
        ...
        this.selectedIds = this.selectableProposals.map(p => p.id)
      }
    },
