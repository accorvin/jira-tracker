/**
 * Tests for PriorityFilterSelector.vue component - following TDD practices.
 */
import { describe, it, expect } from 'vitest'
import { mount } from '@vue/test-utils'
import PriorityFilterSelector from '../components/PriorityFilterSelector.vue'

describe('PriorityFilterSelector', () => {
  const mockFilters = [
    { id: 'abc123', name: 'My Teams', teams: ['Team A', 'Team B'], components: [] },
    { id: 'def456', name: 'Platform Components', teams: [], components: ['UI', 'API'] }
  ]

  function mountComponent(props = {}) {
    return mount(PriorityFilterSelector, {
      props: {
        filters: [],
        activeFilterId: null,
        ...props
  ...
  it('renders "All Issues" when no filter is active', () => {
    const wrapper = mountComponent()

    expect(wrapper.text()).toContain('All Issues')
  })

  it('renders active filter name when a filter is selected', () => {
    const wrapper = mountComponent({
      filters: mockFilters,
      activeFilterId: 'def456'
    })

    const button = wrapper.find('[data-testid="filter-selector-button"]')
    expect(button.text()).toContain('Platform Components')
  })
  ...
  it('shows "All Issues" option in dropdown', async () => {
    const wrapper = mountComponent({ filters: mockFilters })

    await wrapper.find('[data-testid="filter-selector-button"]').trigger('click')

    const allOption = wrapper.find('[data-testid="filter-option-all"]')
    expect(allOption.exists()).toBe(true)
    expect(allOption.text()).toContain('All Issues')
  })
  ...
  it('shows all saved filters in dropdown', async () => {
    ...
    expect(wrapper.text()).toContain('Platform Components')
  })
  ...
  it('emits select-filter with null when "All Issues" is clicked', async () => {
    const wrapper = mountComponent({
      filters: mockFilters,
      activeFilterId: 'abc123'
    })
  ...
  it('highlights "All Issues" when no filter is active', async () => {
    const wrapper = mountComponent({ filters: mockFilters, activeFilterId: null })

    await wrapper.find('[data-testid="filter-selector-button"]').trigger('click')

    const allOption = wrapper.find('[data-testid="filter-option-all"]')
    expect(allOption.classes()).toContain('font-semibold')
  })
